...
export const OCTAGON_API_KEY = process.env.OCTAGON_API_KEY;
export const OCTAGON_API_BASE_URL = process.env.OCTAGON_API_BASE_URL || "https://api.octagonagents.com/v1";

if (!OCTAGON_API_KEY) {
  console.error("Error: OCTAGON_API_KEY is not set in the environment variables");
  console.error("Please set the OCTAGON_API_KEY environment variable");
  process.exit(1);
}

// Initialize OpenAI client with Octagon API
export const octagonClient = new OpenAI({
...
export async function processStreamingResponse(stream: any): Promise<string> {
  let fullResponse = "";
  let citations: any[] = [];
...
export function createServer(): McpServer {
  ...
  // Register all Octagon tools
  registerTools(server);
  
  return server;
}

// Register all tools on the provided server
